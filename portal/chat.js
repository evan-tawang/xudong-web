        ...
        function initPage() {
                ...
                function initChatArea(){
                    ...
                    '<div class="chat_main" id="chatMain">' +
                        '<div class="chat_header">' +
                            '<div id="chatServiceName" class="chat_service_name">客服：' + chatroom.options.staff.name + '</div>' +
            ...
            function createSession() {
                ...
                $.ajax({
                    ...
                    var data = res.data;
                    console.log(data);
                    chatroom.options.visitor.id = data.visitorId;
                    chatroom.options.staff.id = data.staffId;
                    chatroom.options.staff.name = data.otherSideName;
                    chatroom.options.status = data.status;
                    chatroom.options.sessionId = data.id;
                    // 将职员名字设置到职员列表中
                    $('#chatServiceName').html('客服代表：' + chatroom.options.staff.name + '正在为您服务')
                    // 建立连接后获取历史记录
                    getHistoryMsgs();
                    // 建立websocket连接
                    createSocket();
            ...
            function changeFile(event) {
                ...
                reader.onload = function (event) {
                    sendMsg({content: reader.result, contentType: 2})
                };
            ...
            function createSocket() {
                ...
                chatroom.options.socket = stompClient;
            }
            // 时间转换
