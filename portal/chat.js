...
var chatroom = {
    ...
    setOptions(options) { // 设置chatroom的参数
        Object.assign({}, this.options, optiosn);
    },
    instance (options) {
        // 设置各种系统常量
        ...
        function initPage() {
                ...
                function initChatArea(){
                    ...
                    '<div class="chat_main" id="chatMain">' +
                        '<div class="chat_header">' +
                            '<div id="chatServiceName" class="chat_service_name">' + chatroom.options.staff.name + '</div>' +
                        ...
                        '<div class="chat_container">' + 
                            ...
                            '<div class="chat_attachment">' +
                                ...
                                '<div class="chat_image">' +
                                    ...
                                    '<input onchange="changeFile(event)" id="imageFile" type="file" accept=".png,.jpg,.jpeg" style="display: none;">' +
                                '</div>' +
                            '</div>' +
            ...
            function createChatMsg(msg) {
                ...
                dom1.className = 'chat_history_' + getSendTypeLabel(msg.sendUserType);
                // 构建信息内容块
                ...
                titleDom.textContent = msg.name + ' ' + msg.time;
                // 构建信息
            ...
            function getSendTypeLabel(type){
            ...
            function createSession() {
                // 获取历史记录
                $.ajax({
                    url: host + '/chat/createSession',
                    type: 'POST',
                    cache: false,
                    processData: false,
                    contentType: false,
                    data: chatroom.options.custom,
                }).done(function(res) {
                    chatroom.options.visitor.id = res.visitorId;
                    chatroom.options.visitor.ip = res.visitorIp;
                    chatroom.options.staff.id = res.staffId;
                    chatroom.options.staff.name = res.staffName;
                    chatroom.options.status = res.status;
                    chatroom.options.sessionId = res.sessionId;
                    // 将职员名字设置到职员列表中
                    $('#chatServiceName').html(chatroom.options.staff.name)
                    // 建立连接后获取历史记录
                    getHistoryMsgs();
                    // 建立websocket连接
                    createSocket();
            ...
            function sendMsg (params) {
                // TODO
                var data = {
                    sessionId: chatroom.options.sessionId,
                    receiveId: chatroom.options.visitor.id,
                    content: params.content,
                    content: params.contentType
                }
                if (chatroom.options.socket) {
                    chatroom.options.socket.send(data);
                } else {
                    alert('发送失败')
                }
                // $.ajax({
                //     url: host + '/chat/sendMsg',
                //     type: 'POST',
                //     cache: false,
                //     processData: false,
                //     contentType: false,
            ...
            function createSocket() {
                ...
                socket.onmessage = function(e) {
                    ...
                    createChatMsg({
                        ...
                        avatar: data.avatar,
                        name: data.name,
                        time: dateFormat(data.gmtCreate, 'HH:mm:ss'),
                    })
                };
                ...
                chatroom.options.socket = socket;
            }
            // 时间转换
